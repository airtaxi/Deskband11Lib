...
| `Placement`               | `Auto`                      | `Auto`는 가운데 정렬일 때 양쪽 남는 공간 중 더 넓은 쪽에 콘텐츠를 두고, 왼쪽 정렬일 때는 `BeforeNotificationArea`로 동작합니다. `BeforeNotificationArea`는 항상 알림 영역 앞에 둡니다. `BeforeStartButton`은 시작 버튼 앞쪽에 두며, 가운데 정렬이 아니면 `Auto`로 폴백합니다. |
