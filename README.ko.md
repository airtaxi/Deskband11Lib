...
Deskband11Lib는 UI 프레임워크별 NuGet 패키지로 제공됩니다. 작업 표시줄에 붙이는 핵심 로직은 `Deskband11Lib.Core`에 들어 있고, WinUI, WPF, Uno Platform Skia Desktop 패키지는 각 프레임워크에서 쓰기 쉬운 API를 제공합니다. 이후 Avalonia 같은 프레임워크도 같은 방식으로 추가할 수 있습니다.
...
| `Deskband11Lib.Uno.Skia` | Uno Platform Skia Desktop 앱용 패키지입니다. Uno Platform으로 만든 콘텐츠를 같은 방식으로 Windows 11 작업 표시줄에 배치할 수 있습니다.   |
...
Uno Skia에서도 API 흐름은 WinUI와 같습니다. 차이는 facade 네임스페이스뿐이며, 앱은 `Uno.Sdk`와 `net10.0-desktop` 대상 프레임워크, Skia Win32 호스트를 사용해야 합니다.
...
모든 옵션은 `Deskband11Lib.Core.TaskbarContentHostOptions`에 정의되어 있고, WinUI, WPF, Uno Skia 패키지에서 공통으로 사용합니다.
...
- `Deskband11Lib.Uno.Skia.Sample`: Uno Platform Skia Desktop 앱용 샘플입니다.
...
- Uno Platform Skia Desktop은 `Uno.Sdk`와 `net10.0-desktop` 대상 프레임워크가 필요합니다.
