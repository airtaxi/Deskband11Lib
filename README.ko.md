...
Deskband11Lib는 UI 프레임워크별 NuGet 패키지로 제공됩니다. 작업 표시줄에 붙이는 핵심 로직은 `Deskband11Lib.Core`에 들어 있고, WinUI, WPF, Avalonia 패키지가 각 프레임워크에서 쓰기 쉬운 API를 제공합니다. 이후 다른 프레임워크도 같은 방식으로 추가할 수 있습니다.
...
| `Deskband11Lib.Avalonia` | Avalonia 앱용 패키지입니다. Avalonia로 만든 콘텐츠를 같은 방식으로 Windows 11 작업 표시줄에 배치할 수 있습니다.                             |
...
- Avalonia 패키지는 Avalonia 11 데스크톱 앱을 지원합니다.
...
Avalonia에서도 API 흐름은 같습니다. Avalonia `Window`와 `Control`(콘텐츠 요소)를 넘기면 됩니다. Avalonia 11에는 `FrameworkElement`가 없으므로 `Control`을 직접 사용합니다.
...
- `Deskband11Lib.Avalonia.Sample`: Avalonia 11 데스크톱 앱용 샘플입니다.
...
- Avalonia는 Avalonia 11(`Avalonia.Win32` 또는 `Avalonia.Desktop`)과 `Avalonia.Themes.Fluent`이 필요합니다.
