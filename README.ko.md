...
현재 Windows 11에서는 작업 표시줄의 자식 HWND만 살펴봐서는 실제 버튼 폭을 안정적으로 알기 어렵고, 작업 표시줄 콘텐츠 정렬이 왼쪽일 수도 있고 가운데일 수도 있습니다. 그래서 Deskband11Lib는 UI Automation으로 시작 버튼(작업 표시줄 프로세스에 속한 가장 왼쪽 버튼)과 그 오른쪽으로 이어지는 작업 표시줄 앱 버튼 묶음을 함께 감지하여, 정렬 상태와 상관없이 항상 가장 왼쪽 요소 옆에 콘텐츠를 배치합니다. 이 작업은 UI 스레드 밖에서 실행되고 결과도 캐시되므로, 레이아웃을 갱신하는 동안 앱 UI가 멈추지 않습니다.
